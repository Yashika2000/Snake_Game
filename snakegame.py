...
pygame.init()
pygame.display.set_caption("Snake_Game 1.O")
pygame.font.init()
random.seed()

# we will declare global variables
SPEED = 0.36
SNAKE_SIZE = 9
APPLE_SIZE = SNAKE_SIZE
SEPARATION = 10 # Seperation between 2 pixels
SCREEN_HEIGHT = 600
SCREEN_WIDTH = 800
FPS = 25
...
#we have used hw surface which stands for hardware surface refers to using memory on the video card for
# storing draws as opposed to main memory
...
score_msg_size = score_font.size("Socre")
background_color = pygame.Color(0, 0, 0) #black color
...
# to check the boundaries here we are not limiting boundaries like it can pass through sreen and come from other size
...
class snake:
    def __init__(self, x, y):
        ...
        self.y = y
        self.direction = KEY["UP"]
        self.stack  = [] # initally it will be empty
    ...
    # we will make the fuction of crashing when snake eats itself
    ...
    def draw(self,screen):
        pygame.draw.rect(screen,pygame.color.Color("blue"),(self.stack[0].x,self.stack[0].y,SNAKE_SIZE,SNAKE_SIZE),0)
        counter = 1
...
def main():
    ...
    mySnake.move()
    start_segments = 3 # initially we will have 3 segment long snake
